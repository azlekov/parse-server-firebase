...
export default class FirebaseStorageAdapter {
  ...
  constructor() {
    this.directAccess = optional(
      "FIREBASE_STORAGE_DIRECT_ACCESS",
      false
    ) as boolean;
  ...
  async createFile(
    ...
    data: Buffer,
    contentType: string,
    options: { tags: unknown; metadata: unknown }
  ): Promise<void> {
    await this.uploadFile(this.bucket, {
      filename,
      data,
      contentType,
      options,
    ...
    if (
      persists("FIREBASE_THUMBNAILS_SIZES") &&
