...
const generateThumbnail = async (bucket: Bucket, image: File, size: string): Promise<void> => {
    ...
    const sharp = await pipeline.process(sharp => sharp.resize(width === 0 ? undefined : width, height === 0 ? undefined : height))
...
const generateThumbnails = async (bucket: Bucket, image: File): Promise<void> => {
...
export {
    optional, required, persists, isImage, generateThumbnails
}